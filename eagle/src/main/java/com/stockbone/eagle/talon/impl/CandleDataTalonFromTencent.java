...
public class CandleDataTalonFromTencent implements CandleDataTalon {

	private static String URI = "http://qt.gtimg.cn/q=";
	...
	public CandleData fetch(Stock stock, Date date) {
		...
		try {
			...
			if (stockInfo.length < 43) {
				logger.error("Tencent:" + stock.getCode() + "未抓取到数据,请检查参数");
				return null;
			}
			...
			if (stock.getExchange() == Exchange.HK) {// 香港股票
				candleData.setOpen(Float.valueOf(stockInfo[5]));
				candleData.setHigh(Float.valueOf(stockInfo[33]));
				candleData.setLow(Float.valueOf(stockInfo[34]));
				candleData.setClose(Float.valueOf(stockInfo[35]));
				candleData.setVolume(Float.valueOf(stockInfo[36]).longValue());
			} else {// 深证、上证
				candleData.setOpen(Float.valueOf(stockInfo[5]));
				candleData.setClose(Float.valueOf(stockInfo[3]));
				candleData.setHigh(Float.valueOf(stockInfo[41]));
				candleData.setLow(Float.valueOf(stockInfo[42]));
				candleData.setVolume(Long.valueOf(stockInfo[6]) * 100);
				candleData.setAmount(Float.valueOf(stockInfo[37]) * 10000);
			}
			candleData.setTime(date.getTime());
		} catch (ClientProtocolException e) {
			logger.error("Tencent:" + stock.getCode() + "异常");
		} catch (IOException e) {
			logger.error("Tencent:" + stock.getCode() + "异常");
		}
		return candleData;
